...
{
	string cfg_path =
		"/root/.config/obs-studio/plugin_config"; // 文件所在目录
	...
	if (!obs_initialized()) {
		...
		//		obs_add_module_path(
		//			"/workspace/learn/obs/aplugins/tmp2/",
		//			"/usr/local/share/obs/obs-plugins/%module%");
	...
	while (property) {
		...
		if (strcmp(name, "device_id") == 0) {
			...
			for (size_t i = 0; i < count; i++) {
				const char *str = obs_property_list_item_string(
					property, i);
...
int OBSImp::AddVideoScensSource(REC_VIDEO_TYPE rec_video_type)
{
	...
	obs_set_output_source(
		...
		fade_transition_); // 将淡出过度和CHANNEL_TRANSITION 绑定
	//    obs_source_release 1
	obs_source_release(
		fade_transition_); // obs_set_output_source内部已经做了obs_source_addref(source);
	scene_ = obs_scene_create("MyScene"); // 创建场景
	if (!scene_) {
		return -2;
	}
	obs_source_t *s = obs_get_output_source(
		SOURCE_CHANNEL_TRANSITION); // 获取fade_transition
	obs_transition_set(s,
	...
	property_ = obs_properties_first(
		properties); // 第一个属性，后续可以根据他遍历后面的属性
	return 0;
}
...
int OBSImp::SearchRecVideoTargets(REC_VIDEO_TYPE rec_video_type)
{
	...
	while (property_) {
		...
		cout << "rec_type_name:" << rec_type_name << ", name: " << name
		     << endl;
		if (strcmp(name, rec_type_name) == 0) {
			...
			for (size_t i = 0; i < count; i++) {
				if (rec_video_type == REC_VIDEO_WINDOWS) {
					string = obs_property_list_item_string(
						property_, i);
				} else {
...
int OBSImp::UpdateVideoRecItem(const char *target,
			       REC_VIDEO_TYPE rec_video_type)
{
	for (auto item : vec_rec_targets_) {
		if (item == std::string(target)) { //
			if (rec_video_type == REC_VIDEO_DESKTOP) {
				obs_data_set_string(setting_source_, "monitor",
						    target); // 设置源
			} else if (rec_video_type == REC_VIDEO_CAMERA) {
				obs_data_set_string(setting_source_, "dshow",
						    target);
			} else if (rec_video_type == REC_VIDEO_WINDOWS) {
				obs_data_set_string(setting_source_, "window",
						    target);
			} else {
...
int OBSImp::setupLocalRecord(string &file_name)
{
	...
	if (VIDEO_ENCODER_ID == AV_CODEC_ID_H264) {
		obs_data_set_string(settings, "video_settings",
				    "profile=main x264-params=crf=22");
	...
	obs_data_set_int(settings, "scale_height",
			 BASE_HEIGHT);         // 最终视频编码的宽高
